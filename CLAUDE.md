...
- `MAX_SESSION_NAME_LENGTH` and `normalizeSessionName` bound and normalize start, import and rename
  ...
  tmux session string *is* the name, while the rename path's cleared label is `""`. Both read back as the
  automatic label, so the two representations differ only in the stored row.
