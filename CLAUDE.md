...
- `MAX_SESSION_NAME_LENGTH` bounds start, import and rename alike — rename must not be stricter than
  creation — and counts UTF-16 units to match the browser's `maxlength`. Code points would be friendlier to
  emoji, but HTML counts the same units and Kotlin/Native's common stdlib has no `codePointCount`, so a
  code-point bound would need a hand-rolled surrogate scan and would still be overridden by the stricter
  browser cap. A name carrying an ISO control character is rejected; the empty name is valid and means
...
- `SessionUpdateDto` carries `name`, and `patchIfNewer` applies it only when the frame has one. An absent
  `name` means "keep": a daemon older than the field omits it, and `undefined` must not wipe the label the
  snapshot supplied. `""` is not absent and does clear the label back to the automatic one. This is the
  opposite of `model`, whose null is an authoritative clear.
