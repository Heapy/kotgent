...
изначально тоже объявлялся неприкосновенным — **это обещание нарушено осознанно**: волна 3 поймала
**четыре** настоящих бага продукта, тесты на них верны, и ослаблять их нельзя, поэтому починка вошла в
объём (Task 23a). В остальном:
...
- [x] сценарий `sessions`: из реального DOM восстанавливается ожидаемая иерархия каталогов
- [x] смена уровня группировки в Preferences меняет дерево; в футере видна текущая версия
- [x] **выбор строки сайдбара двигает `location.pathname`** на `/s/{id}`, Back возвращает
- [x] сценарий `empty`: первый запуск показывает прямое действие «Start a session»
- [x] залогировать `[replaces] task 9: …` для замещаемых тестов `WebUiServingTest`
...
- [x] поиск: `⌘K`, ввод подстроки, фильтрация, `aria-activedescendant` следует за стрелками, Enter выполняет команду
- [x] лидер-режим: аккорд открывается, мнемоника срабатывает, зарезервированные чорды видимы, но неактивны
- [x] палитра уступает клавиатуру другому диалогу; точки входа, убранные из десктопного хрома, достижимы
- [x] палитра отвечает за тот экран, на котором открыта
- [x] залогировать `[replaces] task 10: …`
...
- [x] tap по backdrop закрывает, tap по панели — нет
- [x] press внутри панели с отпусканием снаружи **не** закрывает (решает геометрия, не target)
- [x] второй контакт не закрывает диалог, начатый первым — инвариант одного `pointerId` на down/up/click
- [x] экран в состоянии busy не закрывается ни жестом, ни backdrop-ом, но Esc и × работают всегда
- [x] перенести объясняющий комментарий (ключевая часть проектного журнала); залогировать `[replaces] task 11: …`
...
- [x] сценарий `terminal`: вертикальный свайп даёт wheel-события и видимый скролл
- [x] горизонтальный свайп жест не захватывает
- [x] свайп не вызывает клавиатуру (фокус не уходит в helper-textarea)
- [x] залогировать `[replaces] task 12: …`
...
- [x] `restart` → сокет переоткрылся и новые байты идут (**не** «прежнее содержимое вернулось»: `stop()` убивает pty-ребёнка)
- [x] закрытие сокета событий из `page.evaluate` → переподключение по его возврату
- [x] ветки «кандидат уничтожен на 4xx» и «кандидат сохранён при недостижимом демоне» — перехватом запросов в браузере, без участия харнесса
- [x] переключение на другую сессию уничтожает кандидата; учесть, что оно теперь ещё и меняет маршрут
- [x] залогировать `[replaces] task 13: …`
...
- [x] FitAddon получает размер за вычетом padding — сверяются `getBoundingClientRect` и `cols`/`rows`, последняя строка не обрезана
- [x] mobile drawer открывается и закрывается; десктопный сайдбар сворачивается, не перегружая мобильный ящик
- [x] карточки не переносят padding на хост терминала; размер шрифта из преференсов меняет `cols`/`rows`
- [x] переключатель уведомлений рисуется акцентом оболочки — computed color в двух состояниях
- [x] все утверждения читают геометрию, не строки `getComputedStyle`; safe-area **не** проверяется
- [x] залогировать `[replaces] task 14: …`
...
- [x] New session: выбор агента в один клик без предвыбранного значения, радио доступны с клавиатуры, запланированные агенты видимы, но не выбираемы
- [x] отсутствующий бинарник агента даёт внятное сообщение, а не молчаливый отказ
- [x] Import адоптирует сессию и не предлагает Shell
- [x] второе действие жизненного цикла отклоняется вслух
- [x] Preferences, Help и Phone access открываются и несут ожидаемое содержимое
- [x] залогировать `[replaces] task 15: …`
...
- [x] key bar: специальные клавиши уходят бинарными кадрами и не крадут фокус у xterm
- [x] загрузка: `setInputFiles` на пикере палитры, uploader харнесса получает ожидаемое имя и содержимое; частичный батч сообщает про каждое упавшее имя
- [x] unicode: аддон **не** запрашивается по умолчанию, после переключения преференса запрос происходит и `term.unicode.activeVersion` меняется
- [x] залогировать `[replaces] task 16: …`
...
- [x] сценарий `task-linked-session`: бейдж рисуется, link/unlink меняет его без перезагрузки
- [x] `emit` и `task <ref> <state>` двигают состояние живьём; notify-edge срабатывает один раз
- [x] перенести объясняющие комментарии; удалить `WebUiTaskBadgeTest.kt` целиком и ужать `WebUiTaskStateTest.kt` до его единственного KEEP-теста — **`WebUiServingTest.kt` не открывать**, свод в реестр делает задача 24; залогировать `[keep] task 17: …` на оставленный тест
...
- [x] доска и её задачи достижимы из палитры; команды исполняются и меняют состояние
- [x] слои реестра команд не конфликтуют между экранами
- [x] перенести комментарии; удалить grep-файл в этом же коммите волны
- [x] залогировать `[replaces] task 18: …` для теста реестра команд в `WebUiServingTest`, если он затронут
...
- [x] сценарий `board`: колонки и карточки отрисованы; `board-empty` даёт пустое состояние
- [x] **drag карточки** — слоп, переживание перерисовки под capture, `pointercancel` без сетевых запросов, порядок PATCH→POST `/move`, модифицированный клик. Chromium-only по той же причине, что light dismiss (факт 4)
- [x] один-колоночный вид на ширине 390 px
- [x] гонка «тот же ref пришёл REST-ответом и фреймом» разрешается newest-rev-wins
- [x] перенести комментарии; ужать `WebUiBoardTest.kt` до его KEEP-тестов (замок словаря классов, `PROJECT_NAME_MAX_LENGTH`) — **`WebUiServingTest.kt` не открывать**; залогировать `[keep] task 19: …` на каждый оставленный тест
...
- [x] каждая колонка и карточка проверяется геометрией и computed color, а не сравнением CSS-строк
- [x] деталь задачи всплывает над доской, а не сжимает её
- [x] удалить `WebUiBoardStyleTest.kt` целиком: его половина замка «словаря классов» живёт в `WebUiBoardTest.kt`, который сохраняет задача 19
...
- [x] сценарий `task-detail`: лента активности подгружается, комментарий добавляется, зависимости отображаются
- [x] × стоит в углу и закрывает панель; layout панели проверяется геометрией
- [x] удалить grep-файл в этом же коммите волны
...
- [x] сценарий `deep-link`: старт браузера прямо на `/s/{id}` и `/tasks/{ref}` открывает нужный экран
- [x] Back/Forward через `popstate` работают в обе стороны
- [x] каждый экран имеет свой маршрут и восстанавливается по перезагрузке
- [x] удалить `WebUiScreenRoutingTest.kt` целиком и ужать `WebUiRouterTest.kt` до его KEEP-тестов (сверка `DEEP_LINK_PARAM` с `sw.js`, запрет `pushState` в обход роутера) — **`WebUiServingTest.kt` не открывать**; залогировать `[keep] task 22: …`

### Task 23: Закрытие волны 3

**Wave:** 3 · **Depends:** 9–22

- [x] сверить `git status --porcelain` с объединением `FILES:` задач 9–22; лишний файл — стоп
- [x] убедиться, что **ни один** участник не открыл `test/transport/WebUiServingTest.kt`
- [x] прогнать `## Validation Commands`
- [x] при падении вернуть «файл участника → ошибка»; при более чем трёх падениях разделить волну на 7+7 и закрыть двумя проходами
- [x] пометить `[x]` чекбоксы задач 9–22, один коммит волны вместе с файлом плана

### Task 23a: Починка четырёх багов продукта, найденных волной 3
...
**Files:**
- Modify: `resources/webui/components/TerminalPane.js`, `resources/webui/app.js`,
  `resources/webui/components/Board.js`, `resources/webui/style.css`

- [x] **unicode-аддоны мертвы:** `lib/unicode.js` пишет `term.unicode.activeVersion`, но `new Terminal({…})` не ставит `allowProposedApi: true`, а `term.unicode` спрятан за `_checkProposedApi` — любой выбор режима бросает. Ловит `MobileFeaturesTest`
- [x] **Preferences не закрывается после Save:** guard `sameForm` отличает чужую правку по ревизии формы, но демон рассылает `preferences_update` про эту же запись, и по петле кадр обгоняет ответ на PUT — защита срабатывает на собственном эхе. Сохранить назначение guard'а, а не удалить его. Ловят `SidebarTest` ×3 и `LayoutTest` ×1
- [x] **drag доски почти неначинаем:** capture берётся только после слопа в 8 px, а ручка `⠿` — примерно 12×16 px, так что курсор уходит с элемента раньше квалифицирующего `pointermove`. Брать capture на `pointerdown`: у ручки нет конкурирующего жеста, ради которого слоп нужен диалогу. Слоп сохранить для отличения клика от перетаскивания. Ловят `BoardTest` ×4 и `BoardStyleTest` ×1
- [x] **`.board-status` съедает указатель над доской:** тост объявлен без `pointer-events: none`, поэтому `elementFromPoint` отвечает им, а не колонкой, и дроп над нижней частью средней колонки не попадает никуда. Ловит `BoardTest`
- [x] `node --check` на каждый изменённый модуль; прогнать `./kotlin task :webuitest:testJvm`
...
нечего. Итог: 4050 → 1111 строк, 49 → 25 тестов в одном файле вместо четырёх; нативная сюита
**1332 / 0 skipped** (1361 − 29), браузерная — 102/102.

**Незакрытый остаток (добит отдельным проходом, см. Task 25).** Один тест уносил треть всего
оставшегося: `daemonServesTheAppEntryModule` — 114 строк и 35 grep'ов по `app.js`, с собственным
комментарием «there is no JS harness, so these greps are what stops the whole feature from being
deleted», который к волне 4 уже был неправдой.

### Task 25: Приёмка

**Wave:** 5 · **Depends:** 24

- [x] проверить, что требования Overview выполнены и ни одного нового grep-утверждения не добавлено
- [x] пересчитать браузерные тесты и сверить с бюджетом 55–70; зафиксировать фактическое время прогона и объём кэша браузеров
- [x] нативная сюита: **0 skipped**, а дельта к 1432 равна числу удалённых тестов
- [x] крайние случаи: отсутствующий `webuicheck.kexe` даёт внятное падение с инструкцией; убитый драйвер не оставляет висящих процессов; сторожевой таймер срабатывает; повторный прогон не конфликтует по портам
- [x] харнесс не создал ничего в `~/.kotgent`, в ФС проекта, на tmux-сокетах и не написал `.kotgent.json`
- [x] прогнать `## Validation Commands`, пометить `[x]`, закоммитить

**Измерено приёмкой (факт).** grep-ярус 10 файлов / 8195 строк / 136 тестов / 1181 `contains(` + 211
`indexOf(` → 3 файла / 1646 строк / 36 тестов / 93 `contains(` + 15 `indexOf(` (`WebUiServingTest.kt`
1111, `SpaRoutingTest.kt` 450 без изменений, новый `WebUiCheckTest.kt` 85). Нативная сюита 1432 → 1332
`@Test`, дельта 100 = 101 удалённый минус 1 добавленный (`WebUiCheckTest`), 0 skipped. Браузерная
102/102, 0 skipped — четыре подряд зелёных прогона, флейков нет. Времена (M-класс, тёплая сборка):
`./kotlin build` 53 с; `./kotlin test` 2:40 (ярусы идут ПАРАЛЛЕЛЬНО, браузерный — критический путь);
`:webuitest:testJvm` в одиночку 2:38 / 2:38 / 2:38; `:kotgent:testMacosArm64Debug` в одиночку 1:39
(47,8 с собственно тестов). Кэш браузеров Playwright: chromium 356 МБ + headless shell 196 МБ + ffmpeg
2,5 МБ; драйвер тянет также firefox (282 МБ) и webkit (294 МБ) — CI-кэш ≈ 1,1 ГБ, а не «Chromium
один». Крайние случаи проверены реально: убранный `webuicheck.kexe` даёт `AssertionError` с текстом
про `./kotlin build` и в нативном тесте (exit 1), и в браузерном (102 failed / 0 skipped за 5,6 с,
причём `:webuitest:testJvm` бинарь НЕ пересобирает); `--exit-after-ms=4000` при незакрытом stdin даёт
exit 4 и сообщение в stderr; SIGKILL драйвера не оставляет сироты (EOF на stdin); порт всегда
эфемерный (`port = 0`) — два харнесса одновременно взяли 58686 и 58687. Безвредность подтверждена
эмпирически: у живого `webuicheck` НОЛЬ открытых файлов под `$HOME` вне чекаута и ноль ссылок на
`.kotgent/`; список `~/.kotgent` до и после идентичен, `git status` чист, `-L kotgent` — 66 сессий до и
после, `-L kotgent-test` сервера не поднимал (его поднимает и гасит НАТИВНЫЙ `TmuxTest`, как и раньше).

**Остаток, добитый после приёмки.** Приёмка нашла одно незакрытое место: `daemonServesTheAppEntryModule`
(114 строк, 35 grep'ов) держал треть всего оставшегося текста файла и утверждал имена идентификаторов,
ПОРЯДОК подстрок внутри тела колбэка (`indexOf` по срезу) и ЧИСЛО вхождений регулярки. Три поведения из
него не были покрыты браузером нигде — они дописаны в `webuitest/test/SidebarTest.kt` (сайдбар рисует все
три: пилюлю непрочитанного на строке, ноту «Loading sessions…» в теле и `#status-line` в подвале):

- `theUnreadBadgeIsTheDaemonsNumberAndALostMarkReadHealsItself` — POST `/read` несёт отображаемый `lastSeq`,
  бейдж двигает только подтверждение демона (локально не обнуляется), `4xx` останавливает петлю, сетевой
  отказ — нет, и оба молчания измерены на ОДНОМ окне в 6 с (три задержки петли), так что тихое окно —
  решение, а не короткое ожидание;
- `theSidebarSaysItIsLoadingUntilTheFirstSnapshotDecidesTheListIsEmpty` — на сценарии `empty` список пуст в
  обоих состояниях, поэтому единственная разница между «Loading sessions…» и «No sessions yet» — приход
  снапшота; отказ вводится в КОНСТРУКТОР сокета (та ветка, что раньше сдавалась на всю жизнь страницы), и
  выздоровление доказывает переподключение, которое grep считал по числу `setTimeout(connect, 2000)`; там же
  живёт негативный пин «ни одного оптового `GET /sessions`» — теперь как счёт запросов, а не как отсутствие
  подстроки;
- `theOutageIsAnnouncedOncePerOutageAndTheRoutineLineOnlyOnce` — латч наблюдаем только через ЧУЖОЕ сообщение
  (два одинаковых `say` не дают мутации DOM вовсе), поэтому тест вклинивает ответ команды «Copy tmux
  command» и требует, чтобы он пережил три следующих неудачных реконнекта; барьером служат сами сокеты.

Все три фальсифицированы снятием защиты в продукте (безусловный `say` в `onclose`, удалённый
`isDefiniteAnswer`, панель «No sessions yet» без `sessionsReady`): падают ровно они трое, каждый —
на утверждении о снятом правиле; продукт возвращён `git checkout`. `daemonServesTheAppEntryModule` ужат до
контракта отдачи (200 + `javascript` + непустое тело + `no-cache`); в реестр source-guard'ов не перенесено
ничего — каждое оставшееся утверждение либо теперь доказывается Chromium, либо является деталью реализации
внутри одного файла. Одно утверждение снято БЕЗ замены и записано в KDoc теста: «никакой null-guard не
фильтрует `msg.model` из патча» — браузер бы это увидел, но у харнесса нет команды, очищающей модель, так
что не хватает фикстуры, а не наблюдаемости. Итог: `WebUiServingTest.kt` 1111 → **1049 строк**, те же 25
тестов, `contains(` 87 → 60 и `indexOf(` 13 → 5 (по всему ярусу — 93 → 66 и 15 → 7); нативная сюита
**1332 / 0 skipped** (без изменений — тестов не удалено и не добавлено), браузерная
**102 → 105 / 0 skipped** — четыре зелёных прогона яруса: три до фальсификации и один после отката.
...
- [x] раздел про ярус: где живут UI-тесты, правило суффикса `Test`, правило геометрии вместо `getComputedStyle`, Chromium по умолчанию и условие добавления WebKit (измеренное расхождение), факт 4 как обоснование Chromium-only для жестов
- [x] записать, почему нативная сюита драйвит харнесс через `--self-check` и почему в нём только cinterop-зависимые проверки
- [x] добавить `fakes`, `webuicheck`, `webuitest` в «Module structure» и «Where things live»
- [x] обновить baseline и записать, что `./kotlin build` обязателен перед `./kotlin test` по двум причинам; записать цену агрегата и быстрые петли
- [x] переписать «There is deliberately no JavaScript test harness» — JS-сборки нет, но поведение проверяется браузером; правку вносить в `CLAUDE.md` (корневой `AGENTS.md` — указатель на него)
- [x] чекбокс «переместить план в `completed/`» пометить `[x]` **без фактического перемещения** — план двигает сам харнесс на своём шаге; перемещение изнутри задачи ломает последующие фазы, читающие путь к плану
- [x] прогнать `## Validation Commands`, закоммитить

**Сделано (факт).** `CLAUDE.md` получил три модуля в «Module structure» и в «Where things live», ярусное
правило («если на вопрос может ответить Chromium — тест живёт в `webuitest/`») с правилом геометрии,
Chromium-only с измеренным обоснованием и условием точечного WebKit, четыре факта драйва
(`Pattern.quote`, один тикет на харнесс, Esc в `<input type="search">`, ожидание наблюдаемого состояния),
обоснование `--self-check` на двух cinterop-проверках, безвредность харнесса, новый baseline с ценой
агрегата и быстрыми петлями, четыре пойманных бага как аргумент и четыре записанных долга. Правило суффикса
`Test` ушло в «Load-bearing toolchain gotchas» — это ловушка сборки, а не яруса. Три инварианта дополнены по
месту, а не пересказаны в разделе тестов: `allowProposedApi` в абзаце про unicode-аддоны,
`pointer-events: none` в абзаце про `.board-status`, и правило «собственное эхо обгоняет собственный ответ»
в абзаце про протокол `/events`. Вне `CLAUDE.md` исправлены два места: комментарий кэша браузеров в
`ci.yml` (кэш ≈1,1 ГБ и содержит ВСЕ движки, включая firefox и webkit — ограничение сознательно не
вводится, причина записана) и `docs/TESTING.md`, который не знал ни про `webuicheck`, ни про `webuitest`,
ни про Playwright (дописан его же нормативным голосом).

**Отклонение от предпоследнего чекбокса, санкционированное явно.** План двигал себя в `completed/` руками
харнесса `/planning:exec`; эта задача исполнялась вне него, поэтому перемещение выполнено здесь и оно
настоящее. Заодно проставлены `[x]` у задач 9–23a и у последнего пункта задачи 25: работа их вся в дереве
и закоммичена (четырнадцать файлов `webuitest/test/`, удалённые восемь grep-файлов, четыре починки
продукта), а закрывающий проход волны 3 не пометил их, потому что первая валидация волны была красной и
дожималась отдельными прогонами.
