...
- [ ] **`.board-status` съедает указатель над доской:** тост объявлен без `pointer-events: none`, поэтому `elementFromPoint` отвечает им, а не колонкой, и дроп над нижней частью средней колонки не попадает никуда. Ловит `BoardTest`
