...
fun runCli(args: Array<String>): Int = when (val command = parseArgs(args.toList())) {
    ...
    is TaskMove -> TaskCommands.move(command.ref, command.target)
    is TaskDep -> TaskCommands.dep(command.ref, command.on, command.remove)
    is TaskDelete -> TaskCommands.delete(command.ref)
